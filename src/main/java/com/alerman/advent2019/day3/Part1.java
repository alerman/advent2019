...
public class Part1 {
    public static void main(String[] args) {
        ...
        Set<Point> line1 = loadSetFromInput(line1Input);
        Set<Point> line2 = loadSetFromInput(line2Input);

        Set<Point> commonPoints = Sets.intersection(line1, line2);
        double min = Double.MAX_VALUE;
        for(Point p : commonPoints)
        {
            min = Math.min(Math.abs(p.getX()) + Math.abs(p.getY()), min);
        }
    ...
    public static Set<Point> loadSetFromInput(String input){
        Set<Point> line = new
                HashSet<>();
        int x=0;
        int y=0;
